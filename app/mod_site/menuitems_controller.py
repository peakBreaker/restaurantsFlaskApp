...
@mod.route('/restaurants/<int:restaurant_id>/menu/new',
           methods=['GET', 'POST'])
@valid_login_session
def new_menuitem(restaurant_id):
    ...
    if request.method == 'POST':
        ...
        success = create_menuitem(name, description, price,
                                  restaurant_id, login_session['user_id'])
        if success:
            flash("New menuitem created!")
            return redirect(url_for("site.show_menuitems",
        ...
        owner = r[1]
        if owner:
            return render_template('menuitems/newmenuitem.html',
...
def show_menuitems(restaurant_id, menuitem_id=None):
    ...
    if 'username' not in login_session:
        return render_template('menuitems/public_menuitems.html',
                               restaurant=restaurant, items=items)
    else:
        return render_template('menuitems/menuitems.html',
                               restaurant=restaurant, items=items, owner=owner)
